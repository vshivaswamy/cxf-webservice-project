...
/**
 * @author vinays 
 * An aspect class to format the exception messages
 */
@Aspect
public class ValidatorAspect {
	...
	public void addErrorstoException(JoinPoint joinPoint, Throwable e)
		...
		if (e instanceof MethodConstraintViolationException) {
			...
			if (exceptionsThrown.length > 0) {
				...
				jsfExceptions = new CreditHistoryValidationException();
			}
			...
			Set<MethodConstraintViolation<?>> parametersViolations = exception.getConstraintViolations();
			...
			if (!parametersViolations.isEmpty()) {
				...
				for (MethodConstraintViolation violation : parametersViolations) {
					...
					if (jsfExceptions instanceof CreditHistoryValidationException) {
						((CreditHistoryValidationException) jsfExceptions)
						.addContextValue(fieldName,	violation.getMessage());
	...
	/**
	 * 
	 * Retrieve arguments for the JoinPoint - if parameters are named, save the names
